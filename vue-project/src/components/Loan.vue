    ...
    <div class="card shadow mb-3" v-if="customer">
        <div class="card-body">
            ...
            <table class="table table-hover table-striped">
                ...
                <tbody>
                    <tr v-for="(deposit, idx) in customer.loans" v-if="customer.loans.length > 0">
                        <th scope="row">{{ deposit.id }}</th>
                        <td>{{ deposit.purpose }}</td>
                        <td>{{ deposit.submitted_on }}</td>
                        <td>{{ deposit.disbursement_on }}</td>
